...
namespace Chatly.Services
{
    public class ChatService: IChatService
    {
        ...
        public async Task<int> GetChatIdFromMessage(int messageId)
        {
            return await _context.Message
                .Where(m => m.Id == messageId)
                .Select(m => m.Topic.ChatId)
                .FirstAsync();
        }

        public async Task<Reaction> ToggleReaction(int messageId, string userId, ReactionType type)
        {
            var existingReaction = await _context.Reaction
                .FirstOrDefaultAsync(r => r.MessageId == messageId && r.CreatedById == userId);
            ...
            {
                _context.Reaction.Remove(existingReaction);
                await _context.SaveChangesAsync();
