...
namespace Chatly.Controllers
{
    ...
    public class ChatController : ControllerBase
    {
        ...
        public async Task<IActionResult> GetTopicMessages([FromQuery] string topicId)
        {
            try
            {
                var parsedTopicId = int.Parse(topicId);
                var userId = GetUserId();

                var chatId = await _chatService.GetChatIdFromTopic(parsedTopicId);

                if (!await _chatService.IsUserInChat(userId, chatId))
                    return Forbid();

                var messages = await _chatService.GetTopicMessages(parsedTopicId);

                // Zwracanie reakcji wraz z wiadomoœciami
